...
namespace Euphorically
{
    public class OldMain : Script
    {
        ...
        private void OnKeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Subtract:
                    ...
                    List<Ped> closestPeds = World.GetNearbyPeds(Game.Player.Character, _debugConfig.PedSearchRadius)
                        .OrderBy(ped => ped.Position.DistanceTo(Game.Player.Character.Position))
                        .ToList();
                    if (closestPeds.Count > 0)
                        Game.Player.Character.Euphoria.PointGun.RightHandTarget = closestPeds.First().Position;
                    Game.Player.Character.Euphoria.PointGun.UseTurnToTarget = true;
                    Game.Player.Character.Euphoria.PointGun.Start();
                    break;
                case Keys.Right:
        ...
        private void OnTick(object sender, EventArgs e)
        {
            ...
            {
                List<Ped> closestPeds = World.GetNearbyPeds(Game.Player.Character, _debugConfig.PedSearchRadius)
                    .OrderBy(ped => ped.Position.DistanceToSquared(Game.Player.Character.Position))
                    .ToList();
                Vector3 target = new Vector3();
                if (closestPeds.Count > 0)
                    target = closestPeds.First().Position;
                target.Z -= 0.5f;
                
                if (closestPeds.Count > 0)
                {
                    Game.Player.Character.Euphoria.PointGun.RightHandTarget = target;
                    character.Euphoria.PointGun.Start();
                }
                else
                {
                    character.Euphoria.PointGun.Stop();
                }
            }
            ...
            if (!_euphoriaCooldownTimer.Completed)
            {
                if (!_cancelCheck)
                {
                    ...
                    if (posDiff.X < 0.005f && posDiff.Y < 0.005f && posDiff.Z < 0.005f)
                    {
        ...
        private void InitializeBulletImpulseConfig(in Ped attackedPed, in Ped attackingPed)
        {
            ...
            attackedPed.Euphoria.ApplyBulletImpulse.Impulse = distanceNormalized * 50000;
        }
